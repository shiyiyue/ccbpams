 ...
 * Date: 2013-01-24
 */
public class CustMngDataModel extends LazyDataModel<CustMngVO> {
    private static final Logger logger = LoggerFactory.getLogger(CustMngDataModel.class);

    private CustMngParam paramBean;
    private CustlistMapper custlistMapper;

    public CustMngDataModel(CustlistMapper custlistMapper, CustMngParam paramBean) {
    ...
    public List<CustMngVO> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        ...
        try {
            ...
            dataList = this.custlistMapper.selectCustMngRecordsByPageSize(paramBean);
        } catch (Exception e) {
        ...
        if (super.getRowCount() <= 0) {
            int total = custlistMapper.countCustMngRecords(paramBean);
