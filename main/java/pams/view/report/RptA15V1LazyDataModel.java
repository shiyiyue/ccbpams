 ...
 * Date: 2015-01-30
 */
public class RptA15V1LazyDataModel extends LazyDataModel<OdsbEbankSign> {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    private BasePagedQryParamBean paramBean;
    private OdsbRptMapper odsbRptMapper;

    public RptA15V1LazyDataModel(OdsbRptMapper odsbRptMapper, BasePagedQryParamBean paramBean) {
        this.odsbRptMapper = odsbRptMapper;
        this.paramBean = paramBean;
    }

    @Override
    public List<OdsbEbankSign> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        List<OdsbEbankSign> dataList;
        try {
            ...
            if (sortField != null) {
                ...
                param.setSortField("SIGN_INSTN");
            }
            dataList = this.odsbRptMapper.selectRptA15V1Records(param);
        } catch (Exception e) {
        ...
        if (super.getRowCount() <= 0) {
            int total = odsbRptMapper.countRptA15V1Records(paramBean);
